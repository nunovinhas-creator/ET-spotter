...
Inclui retry logic com exponencial backoff e fetch paralelo (ThreadPoolExecutor).
"""
...
MAX_WORKERS = 5  # paralelo mas sem saturar a API
...
def fetch_with_retry(symbol: str, max_retries: int = MAX_API_RETRIES) -> pd.DataFrame:
    ...
    raise RuntimeError("fetch_with_retry: loop não executou")  # never reached


def _fetch_and_save(symbol: str, out_dir: Path) -> tuple[str, bool, str]:
    """Tarefa individual para ThreadPoolExecutor."""
    try:
        df = fetch_with_retry(symbol)
        df.to_csv(out_dir / f"{symbol}.csv")
        return symbol, True, f"{len(df)} candles"
    except Exception as e:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(_fetch_and_save, sym, DATA_HOURLY): sym for sym in symbols}
        for future in as_completed(futures):
            symbol, ok, msg = future.result()
            if ok:
                print(f"[OK] {symbol}: {msg}")
                success_count += 1
            else:
                print(f"[ERRO] {symbol}: {msg}", file=sys.stderr)
                error_count += 1

    print(f"[RESUMO] Recolha concluída: {success_count} OK, {error_count} erros")

    # Só falha se 0 símbolos foram recolhidos (problema sistémico de rede)
    if success_count == 0:
        sys.exit(1)
